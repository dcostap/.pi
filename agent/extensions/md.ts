...
function copyToClipboard(text: string): void {
  ...
  if (process.platform === "win32") {
    // Do not pipe UTF-8 through powershell.exe stdin: Windows PowerShell often
    // decodes stdin using the console code page, which turns Unicode into
    // mojibake like "" and "". Write UTF-8 to a temp file and have
    // PowerShell read it explicitly as UTF-8 before placing it on the clipboard.
    const tmpFile = path.join(
      os.tmpdir(),
      `pi-md-clipboard-${process.pid}-${Date.now()}.txt`
    );
    ...
    try {
      ...
      const result = spawnSync(
        ...
        [
          "-NoProfile",
          "-Command",
          `$text = Get-Content -LiteralPath '${psPath}' -Raw -Encoding UTF8; Set-Clipboard -Value $text`,
        ],
        { encoding: "utf8" }
      );

      if (result.status !== 0) {
        const err = result.stderr || result.error?.message || "PowerShell Set-Clipboard failed";
    ...
    handler: async (args, ctx) => {
      ...
      try {
        ...
        if (choice === "Copy to clipboard") {
          ...
          ctx.ui.notify(`Copied to clipboard${suffix}${mode}`, "info");
          return;
        }
        ...
        ctx.ui.notify(`Saved${suffix}${mode}: ${outputFile}`, "info");
      } catch (err) {
