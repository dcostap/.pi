...
async function requestFirecrawl(client: FirecrawlClient, path: string, init?: RequestInit): Promise<any> {
...
export async function getFirecrawlClient(config: ExtensionConfig): Promise<FirecrawlClient | undefined> {
	...
	return { apiKey: config.firecrawlApiKey, apiUrl: FIRECRAWL_API_URL };
}
...
export async function crawlWithFirecrawl(client: FirecrawlClient, url: string, limit = 20) {
	...
	while (Date.now() < deadline) {
		const status = await requestFirecrawl(client, `/v2/crawl/${encodeURIComponent(id)}`);
